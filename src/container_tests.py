...
def test_should_not_provide_optional_service():
    class ServiceA:
        def __init__(self):
            pass
    ...
    @injectable()
    class ServiceB:
        def __init__(self, a: Optional[ServiceA]):
            self.a = a

    @injectable()
    class ServiceC:
        def __init__(self, b: Optional[ServiceB]):
            self.b = b
    ...
    assert c.b.a is None


def test_example_multitenant_report_builder():
    root = Container()
    container_a = Container(parent=root)
    container_b = Container(parent=root)
    container_c = Container(parent=root)

    class Logger:
        def __init__(self, context: Dict[str, str]):
            self.context = context

    root.register(Logger, Logger, False)
    root.context[Logger] = Logger({'scope': 'global'})

    container_a.context[Logger] = Logger({'scope': 'A'})
    container_b.context[Logger] = Logger({'scope': 'B'})

    class ApiClientA:
        def __init__(self, log: Logger):
            self.log = log

    container_a.register(ApiClientA, ApiClientA, True)

    class ReportBuilderA:
    ...
    container_b.register(ApiClientB, ApiClientB, True)

    class ReportBuilderB:
    ...
    builder_a = container_a.get(ReportBuilderA)
    builder_b = container_b.get(ReportBuilderB)

    assert builder_a.log is builder_a.provider.log
    assert builder_a.log.context == {
        'scope': 'A'
    }

    assert builder_b.log is builder_b.provider.log
    assert builder_b.log.context == {
        'scope': 'B'
    }

    builder_c = container_c.get(ReportBuilderC)
    assert builder_c.log is builder_c.provider.log
    assert builder_c.log.context == {
        'scope': 'global'
    }
